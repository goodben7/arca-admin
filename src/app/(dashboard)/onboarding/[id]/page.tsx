...
export default function OnboardingDetailPage() {
    ...
    return (
        <PageShell>
            <PageHeader
                title={`Onboarding — ${employeeName || process.employee}`}
                    ...
                    {(process.status === ONBOARDING_PROCESS_STATUS.IN_PROGRESS || process.status === ONBOARDING_PROCESS_STATUS.PENDING) && (
                        <Card className="p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <Button variant="outline" size="sm" className="w-full gap-2 text-rose-600 border-rose-200 hover:bg-rose-50" onClick={() => handleProcessAction('cancel')} disabled={!!acting}>
                                    {acting === 'processcancel' ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4" />}
                                    Annuler
                                </Button>
                            </div>
                        </Card>
