...
export default function DashboardPage() {
    ...
    const pendingTotal = pendingLeaves + pendingRecruitments + pendingTrainings + pendingApplications;
    ...
    const alerts = useMemo(() => {
        ...
        if (expiringContracts.length > 0) {
            list.push({ icon: AlertCircle, label: `${expiringContracts.length} contrat${expiringContracts.length > 1 ? 's' : ''} à renouveler`, href: '/contracts' });
        }
        if (pendingLeaves > 0) list.push({ icon: Clock, label: `${pendingLeaves} congé${pendingLeaves > 1 ? 's' : ''} à valider`, href: '/leave' });
        if (pendingRecruitments > 0) list.push({ icon: UserPlus, label: `${pendingRecruitments} demande${pendingRecruitments > 1 ? 's' : ''} de recrutement`, href: '/recruitment' });
        if (pendingTrainings > 0) list.push({ icon: GraduationCap, label: `${pendingTrainings} formation${pendingTrainings > 1 ? 's' : ''} à valider`, href: '/training' });
        if (pendingApplications > 0) list.push({ icon: ClipboardList, label: `${pendingApplications} candidature${pendingApplications > 1 ? 's' : ''} à traiter`, href: '/recruitment' });
        return list;
        ...
        <PageShell ambient className="space-y-6 dashboard-view">
            ...
            <section aria-label="Indicateurs clés">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                    <DashboardQuickLink label="Congés" value={pendingLeaves} href="/leave" icon={Umbrella} />
                    <DashboardQuickLink label="Offres" value={openJobOffers} href="/job-offers" icon={Briefcase} />
                    <DashboardQuickLink label="Sessions" value={plannedSessions} href="/training/sessions" icon={GraduationCap} />
                    <DashboardQuickLink label="Formations" value={pendingTrainings} href="/training" icon={BookOpen} />
                </div>
            </section>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                ...
                <div className="xl:col-span-4 space-y-4">
                    ...
                    <DataPanel className="!rounded-lg" title="Pipeline recrutement" contentClassName="pt-2">
                    ...
                    <DataPanel
                        ...
                        toolbar={
                            <Link href="/training/sessions" className="text-xs font-medium text-primary-500 hover:text-primary-600 inline-flex items-center gap-1">
                        ...
                        {upcomingSessions.length === 0 ? (
                            ...
                            <ul className="divide-y divide-border-subtle">
                                {upcomingSessions.map((s, i) => (
                                    <li key={i}>
                                        <Link href={`/training/sessions/${s.id}`} className="flex items-center gap-3 px-5 py-3.5 hover:bg-muted/40 transition-colors">
