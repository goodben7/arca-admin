...
function ObjectivesPageContent() {
    ...
    const filtered = useMemo(() => {
        ...
        if (cycleFilter) {
            list = list.filter(o => matchesCycleId(cycleRefOf(o), cycleFilter));
        }
    ...
    }, [objectives, search, employeeFilter, cycleFilter, employees]);
    ...
    const handleCreate = async () => {
        ...
        if (!form.specific.trim() || !form.measurable.trim()) {
            return toast.error('Les champs Spécifique et Mesurable sont obligatoires.');
        }
        if (!form.dueDate) {
            return toast.error("L'échéance est obligatoire.");
        }
        try {
            setCreating(true);
                            ...
                            <TableBody>
                                {filtered.length === 0 ? (
                                    ...
                                    <TableRow key={o.id} className="cursor-pointer hover:bg-secondary-50/60" onClick={() => setSelectedObjective(o)}>
                                        ...
                                        <TableCell className="px-6 text-secondary-500 text-sm">{cycleName(o.cycle || o.evaluationCycleId || o.evaluationCycle)}</TableCell>
                        ...
                        <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="bg-white rounded-2xl shadow-float w-full max-w-lg p-6 space-y-5 max-h-[90vh] overflow-y-auto">
                                <div className="flex items-center justify-between">
                                    <Dialog.Title className="text-lg font-bold text-secondary-900">Nouvel objectif mesurable</Dialog.Title>
