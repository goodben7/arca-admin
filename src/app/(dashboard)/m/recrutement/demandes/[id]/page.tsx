...
export default function RecruitmentDetailsPage() {
    ...
    useEffect(() => {
        if (!request || String(request.status) !== STATUS_APPROVED) {
            setLinkedOffer(null);
            return;
        }
        async function fetchOffer() {
            try {
                ...
                const reqRef = request!.id;
                const match = list.find((o: JobOffer) => {
                    const rr = extractId(o.recruitmentRequest) || o.recruitmentRequest;
                    return rr === reqRef || rr === request!['@id'];
                });
                setLinkedOffer(match || null);
            } catch {
                setLinkedOffer(null);
            }
        }
    ...
    if (error || !request) {
        return (
            <div className="p-10 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <Button variant="outline" onClick={() => router.push('/m/recrutement/demandes')}>
                    Retour
                </Button>
            </div>
        );
    ...
    return (
        <PageShell>
            <PageHeader
                ...
                backHref="/m/recrutement/demandes"
                actions={
                    statusInfo ? (
                        <Badge
            ...
            {String(request.status) === STATUS_APPROVED && (
                <div className="mb-6 p-4 rounded-xl border border-emerald-100 bg-emerald-50/60 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <p className="font-semibold text-emerald-900 text-sm">Demande approuvée</p>
                        <p className="text-sm text-emerald-800/80 mt-0.5">
                            {linkedOffer
                                ? 'Une offre d’emploi a été générée. Publiez-la pour recevoir des candidatures.'
                                : 'L’offre est en cours de génération côté backend. Actualisez dans quelques instants.'}
                        </p>
                    </div>
                    {linkedOffer && (
                        <Link href={`/m/recrutement/offres/${linkedOffer.id}`}>
                            <Button variant="pill" size="sm" className="gap-2 shrink-0">
                                Voir l’offre <ArrowLeft className="w-4 h-4 rotate-180" />
                            </Button>
                        </Link>
                    )}
                </div>
