...
import { Plus, Loader2, AlertCircle, BriefcaseBusiness, Search, X, Trash2, Pencil } from 'lucide-react';
...
import { getJobRoles, createJobRole, updateJobRole, getJobFamilies, getGrades } from '@/lib/api/jobArchitecture';
import { createJobRoleRequiredSkill, deleteJobRoleRequiredSkill, getJobRoleRequiredSkills, getSkills } from '@/lib/api/skill';
import { toIri, relationName, extractId } from '@/lib/api-iri';
...
export default function JobRolesPage() {
    ...
    const openEdit = (role: JobRole) => {
        ...
        setForm({
            title: role.title || '',
            code: role.code || '',
            jobFamily: extractId(role.jobFamily as string) || '',
            grade: extractId(role.grade as string) || '',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editingRole) {
                ...
                toast.success('Fiche métier mise à jour.');
            } else {
    ...
    const openSkillsModal = async (role: JobRole) => {
        ...
        try {
            ...
            if (catalog.length > 0) {
                const used = new Set(req.map(r => extractId(r.skill as string)));
                const first = catalog.find(s => !used.has(s.id));
                if (first) setSkillForm(p => ({ ...p, skillId: first.id }));
            }
        } catch (e: unknown) {
    ...
    const availableSkills = useMemo(() => {
        const used = new Set(requiredSkills.map(r => extractId(r.skill as string)));
        return skillsCatalog.filter(s => !used.has(s.id));
    }, [skillsCatalog, requiredSkills]);
    ...
    const handleAddRequiredSkill = async () => {
        ...
        try {
            ...
            await createJobRoleRequiredSkill({
                jobRole: skillsRole.id,
                skill: skillForm.skillId,
                minimumLevel: skillForm.requiredLevel,
                mandatory: skillForm.mandatory,
            });
            const refreshed = await getJobRoleRequiredSkills(skillsRole.id);
            setRequiredSkills(refreshed);
            const used = new Set(refreshed.map(r => extractId(r.skill as string)));
            const next = skillsCatalog.find(s => !used.has(s.id));
            setSkillForm({ skillId: next?.id || '', requiredLevel: 'INTERMEDIATE', mandatory: true });
    ...
    const handleRemoveRequiredSkill = async (rs: JobRoleRequiredSkill) => {
        if (!skillsRole) return;
        try {
            setRemovingSkillId(rs.id);
            await deleteJobRoleRequiredSkill(rs.id);
                            ...
                            <Dialog.Panel className="bg-white rounded-2xl shadow-float w-full max-w-lg p-6 space-y-5 max-h-[90vh] overflow-y-auto">
                                <div className="flex items-center justify-between">
                                    <Dialog.Title className="text-lg font-bold text-secondary-900">
                                        {editingRole ? 'Modifier la fiche métier' : 'Nouvelle fiche métier'}
                                    </Dialog.Title>
                                ...
                                <div className="flex justify-end gap-3 pt-2">
                                    ...
                                    <Button variant="pill" onClick={handleSave} disabled={creating}>
                                    ...
                                    <>
                                        <div className="space-y-2 max-h-48 overflow-y-auto">
                                            {requiredSkills.length === 0 ? (
                                                ...
                                                <div key={rs.id} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-secondary-100 bg-secondary-50/50 text-sm">
                                                    <div className="min-w-0">
                                                        <p className="font-medium truncate">{skillName(rs.skill as string)}</p>
                                                        <p className="text-xs text-secondary-500">
                                                        {SKILL_LEVEL_LABELS[(rs.minimumLevel || rs.requiredLevel) as SkillLevel] || rs.minimumLevel || rs.requiredLevel}
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={removingSkillId === rs.id}
                                                        onClick={() => handleRemoveRequiredSkill(rs)}
                                                    >
                                                        {removingSkillId === rs.id
                                            ...
                                            {availableSkills.length === 0 ? (
                                                <p className="text-sm text-secondary-500">
                                                    {skillsCatalog.length === 0
                                                        ? 'Aucune compétence dans le catalogue. Créez-en d’abord dans Personnel → Compétences.'
                                                        : 'Toutes les compétences du catalogue sont déjà liées à cette fiche.'}
                                                </p>
                                            ) : (
                                                <>
                                                    ...
                                                    <Button variant="pill" className="w-full" onClick={handleAddRequiredSkill} disabled={addingSkill || !skillForm.skillId}>
