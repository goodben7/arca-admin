...
import { Plus, Loader2, AlertCircle, Layers, X, Pencil } from 'lucide-react';
...
import { getJobFamilies, createJobFamily, updateJobFamily } from '@/lib/api/jobArchitecture';
...
export default function JobFamiliesPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            if (editing) {
                await updateJobFamily(editing.id, form);
                toast.success('Famille de métier mise à jour.');
            } else {
                            ...
                            {families.length === 0 ? (
                                ...
                                <TableRow key={f.id}>
                                    ...
                                    <TableCell className="px-6 text-right">
                                        <Button variant="ghost" size="sm" className="gap-1" onClick={() => openEdit(f)}>
                            ...
                            <Dialog.Panel className="bg-white rounded-2xl shadow-float w-full max-w-md p-6 space-y-5">
                                <div className="flex items-center justify-between">
                                    <Dialog.Title className="text-lg font-bold text-secondary-900">
                                        {editing ? 'Modifier la famille' : 'Nouvelle famille de métier'}
                                    </Dialog.Title>
